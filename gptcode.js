  ...
  Events.on(engine, "collisionStart", (event) => {
    event.pairs.forEach((collision) => {
      ...
      if (bodyA.index === bodyB.index) {
        ...
        // 난이도 조절
        if (numFruits < NUM_FRUITS_LIMIT) numFruits++;
      }
...
// 사용자가 선택한 옵션에 따라 게임을 시작하는 함수
function startGame(timeLimited) {
    if (timeLimited) {
      startTimedGame();
    } else {
      startUntimedGame();
  ...
  // 시간 제한이 있는 게임을 시작하는 함수
  function startTimedGame() {
    const TIME_LIMIT = 300000; // 60초 (단위: 밀리초)
  ...
  // 시간 제한이 없는 게임을 시작하는 함수
  function startUntimedGame() {
    // 시간 제한 없이 게임 진행
    playGame();
  }
  
  // 게임 시작 전에 사용자에게 옵션을 선택하도록 하는 함수
  function promptGameOptions() {
    const timeLimited = confirm("시간 제한이 있는 게임으로 진행하시겠습니까?");
    startGame(timeLimited);
  }
